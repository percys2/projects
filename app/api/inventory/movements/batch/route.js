...
export async function POST(req) {
  try {
    ...
    const { type, branchId, notes, providerName, providerInvoice } = body;
    
    // Accept both 'items' (from BatchEntryModal) and 'movements' for compatibility
    const movements = body.items || body.movements;
    const invoiceNumber = providerInvoice || body.invoiceNumber;

    if (!movements || !Array.isArray(movements) || movements.length === 0) {
      return NextResponse.json({ error: "Se requiere un array de productos (items)" }, { status: 400 });
    }

    if (!type) {
      return NextResponse.json({ error: "Se requiere el tipo de movimiento" }, { status: 400 });
    }
