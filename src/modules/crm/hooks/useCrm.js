...
export function useCrm(orgSlug) {
  ...
  const stats = useMemo(() => {
    ...
    const conversionRate = recentWon + recentLost > 0
  ...
  const clientsByStage = useMemo(() => {
    ...
    opportunities.filter((o) => o.status === "open").forEach((opp) => {
      ...
      const existing = clientMap.get(opp.client_id);
      const stage = stages.find((s) => s.id === opp.stage_id);

      if (!existing || (stage && existing.stageOrder < stage.sort_order)) {
        clientMap.set(opp.client_id, {
    ...
    return Array.from(clientMap.values()).sort((a, b) => a.stageOrder - b.stageOrder);
  }, [opportunities, stages]);
  ...
  const saveOpportunity = useCallback(async (data) => {
    try {
      ...
      if (!res.ok) throw new Error("Error al guardar oportunidad");
  ...
  const moveOpportunityToStage = useCallback(async (opportunityId, newStageId) => {
  ...
  const markOpportunityWon = useCallback(async (opportunityId) => {
    return saveOpportunity({ id: opportunityId, status: "won" });
  }, [saveOpportunity]);

  const markOpportunityLost = useCallback(async (opportunityId, reason) => {
    return saveOpportunity({ id: opportunityId, status: "lost", lost_reason: reason });
  }, [saveOpportunity]);
  ...
  const saveActivity = useCallback(async (data) => {
    try {
  ...
  const saveClient = useCallback(async (data) => {
    try {
      const method = data.id ? "PUT" : "POST";
