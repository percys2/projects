    ...
    <div className="hidden lg:block bg-white rounded-xl border overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-500">Cargando ventas...</td></tr>
            ) : error ? (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-red-500">Error: {error}</td></tr>
            ) : sales.length === 0 ? (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-400">No hay ventas</td></tr>
            ) : (
              sales.map((sale) => (
